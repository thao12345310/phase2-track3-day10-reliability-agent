...
def _evaluate_scenario(name: str, result: RunMetrics) -> str:
    """Return 'pass' or 'fail' based on per-scenario expected behavior."""
    if name == "primary_timeout_100":
        # All traffic should fallback; circuit must open at least once
        return "pass" if result.circuit_open_count > 0 and result.fallback_success_rate > 0.7 else "fail"
    if name == "primary_flaky_50":
        # Circuit should oscillate — at least one open transition expected
        return "pass" if result.circuit_open_count > 0 else "fail"
    if name == "cache_stale_candidate":
        # Cache should be active and false-hit guardrails should fire
        return "pass" if result.successful_requests > 0 else "fail"
    if name == "all_healthy":
        # Near-perfect availability when all providers are healthy
        return "pass" if result.availability >= 0.9 else "fail"
    return "pass" if result.successful_requests > 0 else "fail"
