...
class ResponseCache:
    """Simple in-memory cache with TTL, privacy guardrails, and false-hit detection."""
    ...
    def similarity(a: str, b: str) -> float:
        """Similarity using exact-match fast path + character bigram overlap."""
        a_norm = a.lower().strip()
        b_norm = b.lower().strip()
        if a_norm == b_norm:
            return 1.0
        # Character bigrams capture partial word matches better than token Jaccard
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        """Look up a cached response from Redis with exact-match then similarity scan."""
        if _is_uncacheable(query):
            return None, 0.0
        try:
            exact_key = f"{self.prefix}{self._query_hash(query)}"
            exact_hit = self._redis.hget(exact_key, "response")
            ...
            if best_score >= self.similarity_threshold:
                if _looks_like_false_hit(query, best_cached_query):
                    self.false_hit_log.append(
                        {"query": query, "cached_query": best_cached_query, "score": best_score}
                    )
                    return None, best_score
                return best_value, best_score
            return None, best_score
